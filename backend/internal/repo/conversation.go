...
func (r *ConversationRepo) ListHistoryByUserID(ctx context.Context, userID string) ([]ConversationHistoryRecord, error) {
	if r == nil || r.db == nil {
		return nil, errors.New("conversation repository is not initialized")
	}
	...
	err := r.db.WithContext(ctx).
		...
		Select(`
			...
			COALESCE((
				SELECT m2.content
				FROM messages AS m2
				WHERE m2.conversation_id = c.id AND m2.user_id = c.user_id
		...
		Joins("LEFT JOIN messages AS m ON m.conversation_id = c.id AND m.user_id = c.user_id").
		Where("c.user_id = ?", userID).
		Group("c.id, c.user_id, c.title, c.agent_type, c.created_at, c.updated_at").
